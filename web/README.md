# The Docket — web front end

`docket.html` is the published-Artifact source for **The Docket**, the
visual/interactive face of the Command Center:
...
- Lets you file a new matter directly on the page. Submitting calls Claude
  ...
  Director roster and governance rules as `config.yaml`, and writes the
  result to the artifact's own `db` capability — a shared, live document
  store scoped to this artifact.
- Shows every filed matter in the Case Log: which Directors were
  dispatched and why, the drafted output, any HITL flags (strategic,
  pedagogical, cost-bearing, external-publish), and an Approve / Needs
  Revision control you work through by hand.
- Flags — rather than fakes — anything that needs a live connector
  ...
  flagged case tells you what to bring back to chat instead of pretending
  to have done it.
...
- It is **not** wired to Lovable, ElevenLabs, Granola, or Notion directly —
  ...
  web page. The Docket is honest about that boundary rather than faking it.
- It does **not** read or write the repo's `memory/`, `qa_logs/`, or
  Airtable pipeline directly — its Case Log is its own record, stored in
...
- It does **not** change `config.yaml`'s `operating_mode` — filing a
  matter here is still an attended action (you're the one clicking the
